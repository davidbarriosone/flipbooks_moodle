...
class mod_flipbook_mod_form extends moodleform_mod {
    ...
    function definition() {
        ...
        $mform->addRule('autoflip', null, 'numeric', null, 'client');
    ...
    function validation($data, $files) {
        ...
        if (!isset($this->current->instance) || !$this->current->instance) {
            ...
            if (empty($draftfiles)) {
                $errors['pdffile'] = get_string('nopdffile', 'mod_flipbook');
            }
        }
        
        // Validate dimensions
        if (isset($data['width']) && $data['width'] < 400) {
            $errors['width'] = get_string('error') . ': Width must be at least 400px';
        }
        
        if (isset($data['height']) && $data['height'] < 300) {
            $errors['height'] = get_string('error') . ': Height must be at least 300px';
        }
        
        if (isset($data['autoflip']) && $data['autoflip'] < 0) {
            $errors['autoflip'] = get_string('error') . ': Auto-flip must be 0 or positive';
        }

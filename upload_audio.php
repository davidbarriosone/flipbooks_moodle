...
$audioareaid = required_param('audioareaid', PARAM_INT);
...
$audioarea = $DB->get_record('flipbook_audioarea', array('id' => $audioareaid), '*', MUST_EXIST);

require_login($course, true, $cm);
$context = context_module::instance($cm->id);
require_capability('mod/flipbook:managemultimedia', $context);

$PAGE->set_url('/mod/flipbook/upload_audio.php', array('cmid' => $cmid, 'audioareaid' => $audioareaid));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['audiofile'])) {
    ...
    if ($_FILES['audiofile']['error'] === UPLOAD_ERR_OK) {
        ...
        $fs->delete_area_files($context->id, 'mod_flipbook', 'audio', $audioareaid);
        ...
        $DB->update_record('flipbook_audioarea', $audioarea);
        
        redirect(
            new moodle_url('/mod/flipbook/view.php', array('id' => $cmid)),
            get_string('audiosaved', 'mod_flipbook'),
            null,
...
echo '<small style="display: block; margin-top: 5px; color: #666;">Página ' . $audioarea->pagenumber . ' - Área: ' . round($audioarea->width, 1) . '% × ' . round($audioarea->height, 1) . '%</small>';
echo '</div>';

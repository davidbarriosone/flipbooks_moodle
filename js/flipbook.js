...
(function($) {
    ...
    const CONTROLS_HIDE_DELAY = 3000; // 3 segundos de inactividad
    ...
    // Zoom state
    let isZoomed = false;
    let zoomLevel = 2.5; // Nivel de zoom
    let zoomX = 0;
    let zoomY = 0;
    let isPanning = false;
    let panStartX = 0;
    ...
    function enterFullscreen() {
        ...
        setTimeout(function() {
            ...
            // Iniciar auto-hide de controles
            showControls();
        }, 100);
    }
    ...
    function exitFullscreen() {
        ...
        // Resetear zoom si está activo
        if (isZoomed) {
            zoomOut();
        }

        // Limpiar timeout de auto-hide
    ...
    function showControls() {
        ...
        if (controls) {
            ...
            // Set new timeout to hide controls (solo si no está en zoom)
            if (!isZoomed) {
    ...
    // Setup fullscreen mouse movement detection
    ...
    function handleDoubleTap(e) {
        ...
        // No hacer zoom si se clickea un área de audio u otros elementos interactivos
        if (e.target.classList.contains('audio-area') ||
            e.target.closest('.audio-area') ||
            e.target.closest('.audio-control-bar') ||
        ...
        if (tapGap < DOUBLE_TAP_DELAY && tapGap > 0) {
            ...
            if (!isZoomed) {
                // Calcular punto de zoom basado en donde se hizo click
                const flipbook = document.getElementById('flipbook');
            ...
            lastTapTime = 0; // Reset para evitar triple click
        } else {
    ...
    // Zoom in
    function zoomIn(clickX, clickY, rect) {
        isZoomed = true;
        const flipbook = document.getElementById('flipbook');

        // Calcular la posición relativa del click (0 a 1)
        const relativeX = (clickX - rect.left) / rect.width;
        const relativeY = (clickY - rect.top) / rect.height;

        // Calcular el desplazamiento para centrar el zoom en el punto clickeado
        zoomX = -(relativeX - 0.5) * rect.width * (zoomLevel - 1);
        zoomY = -(relativeY - 0.5) * rect.height * (zoomLevel - 1);

        flipbook.classList.add('zoomed');
        applyZoomTransform();

        // Deshabilitar turn.js mientras está en zoom
        $('#flipbook').turn('disable', true);

        // Ocultar controles cuando se hace zoom
        hideControls();

        console.log('Zoomed in at:', relativeX, relativeY);
    }
    ...
    function zoomOut() {
        ...
        // Mostrar controles cuando se sale del zoom
        if (isFullscreen) {
            showControls();
        }
    ...
    function applyZoomTransform() {
        ...
        if (isZoomed) {
            flipbook.style.transform = `translate(${zoomX}px, ${zoomY}px) scale(${zoomLevel})`;
            flipbook.style.cursor = 'grab';
        } else {
            flipbook.style.transform = '';
            flipbook.style.cursor = '';
        }
    }
    ...
    // Pan move
    function panMove(e) {
        if (!isPanning || !isZoomed || !isFullscreen) return;
    ...
    function setupZoomListeners() {
        ...
        // Double click/tap
        flipbook.addEventListener('click', handleDoubleTap);
        flipbook.addEventListener('touchstart', handleDoubleTap);

        // Panning with mouse
        flipbook.addEventListener('mousedown', function(e) {
            // No iniciar panning si se clickea un área de audio u otros elementos interactivos
            if (e.target.classList.contains('audio-area') ||
                e.target.closest('.audio-area') ||
                e.target.closest('.audio-control-bar') ||
                e.target.tagName === 'BUTTON') {
        ...
        flipbook.addEventListener('touchstart', function(e) {
            // No iniciar panning si se toca un área de audio u otros elementos interactivos
            const target = e.touches[0].target;
            if (target.classList.contains('audio-area') ||
                target.closest('.audio-area') ||
                target.closest('.audio-control-bar') ||
        ...
        flipbook.addEventListener('touchend', stopPanning);
    }

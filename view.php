...
// Only load JS if we have a PDF file
if ($hasfile) {
    // Add CSS and JS
...
// Check if PDF exists
if (!$hasfile) {
    echo $OUTPUT->notification(get_string('nopdffile', 'mod_flipbook'), 'error');
    
    // Show edit link for teachers
    if (has_capability('mod/flipbook:addinstance', $context)) {
        $editurl = new moodle_url('/course/modedit.php', array('update' => $cm->id, 'return' => 1));
        echo html_writer::tag('p', html_writer::link($editurl, get_string('edit')));
    }
...
<div id="flipbook-container">
    ...
    <div id="flipbook-controls">
        ...
        <input type="text" id="page-input" size="3" value="1" />
